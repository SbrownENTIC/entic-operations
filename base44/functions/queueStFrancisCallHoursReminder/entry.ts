...
const REMINDER_NAME = 'St. Francis Call Hours Reminder';
const TO_RECIPIENTS = 'BTessema@enticmd.com;BWycherly@enticmd.com;ealday@enticmd.com;HWang@enticmd.com;jchiu@enticmd.com;KRutherford@enticmd.com;RDrake@enticmd.com;SBrown@enticmd.com;SWolfe@enticmd.com';
const CC_RECIPIENTS = 'Steve.brown@enticmd.com;HEldridge@enticmd.com';
const ALL_RECIPIENTS = [
  'BTessema@enticmd.com',
  'BWycherly@enticmd.com',
  'ealday@enticmd.com',
  'HWang@enticmd.com',
  'jchiu@enticmd.com',
  'KRutherford@enticmd.com',
  'RDrake@enticmd.com',
  'SBrown@enticmd.com',
  'SWolfe@enticmd.com',
...
function buildBody() {
  ...
  <p style="margin: 0 0 12px 0;">This is your reminder to enter your St. Francis call hours into the TRACE (Dynafios) system for last month.</p>
...
async function ensureReminder(base44, sendDate) {
  ...
  const reminderData = {
    ...
    email_subject: `St. Francis Call Hours – ${previousMonthName(sendDate)}`,
    email_body: 'Good morning ENTIC Docs,<br><br>This is your reminder to enter your St. Francis call hours into the TRACE (Dynafios) system for last month.',
  ...
  try {
    ...
    if (!/^\d{4}-\d{2}-01$/.test(sendDate)) {
      return Response.json({
        success: true,
        skipped: true,
        message: 'St. Francis Call Hours reminders only queue on the 1st day of each month.',
        send_date: sendDate
      });
    }

    const reminder = await ensureReminder(base44, sendDate);
    const subject = `St. Francis Call Hours – ${previousMonthName(sendDate)}`;
    ...
    if (existing) {
      return Response.json({
        ...
        bcc: existing.bcc || '',
        send_date: sendDate,
        message: `St. Francis Call Hours reminder for ${sendDate} is already ${existing.status === 'Ready to Send' ? 'queued' : existing.status.toLowerCase()}.`
      });
    }
    ...
    return Response.json({
      ...
      send_date: sendDate,
      next_send_date: addMonths(sendDate, 1),
      message: 'St. Francis Call Hours reminder queued successfully.'
    });
  } catch (error) {
