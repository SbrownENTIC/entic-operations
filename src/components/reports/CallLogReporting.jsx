...
export default function CallLogReporting() {
  ...
  // CallLogUserConfig: user_name field | CallLogUserSummary: user field | benchmark flag: include_in_benchmark
  const norm = (s) => (s||"").trim().toLowerCase().replace(/\s+/g, " "); const cdrMap = React.useMemo(() => { const m = {}; cdrUserStats.forEach(s => { if(s.user_name) m[norm(s.user_name)] = {t: Number(s.inbound_calls||0), a: Number(s.inbound_answered||0)}; }); return m; }, [cdrUserStats]); const benchmarkSet = React.useMemo(() => { console.log("CONFIG_NAMES_SAMPLE", allUserConfigs.slice(0,5).map(c=>({user_name:c.user_name,include_in_benchmark:c.include_in_benchmark}))); console.log("SUMMARY_NAMES_SAMPLE", userSummaries.slice(0,5).map(u=>({user:u.user}))); const s = new Set(allUserConfigs.filter(c => c.include_in_benchmark === true).map(c => norm(c.user_name))); console.log("BENCHMARK_SET_SIZE", s.size, [...s].slice(0,5)); return s; }, [allUserConfigs, userSummaries]);
  const enrichedSummaries = React.useMemo(() => { const enriched = userSummaries.filter(u => benchmarkSet.has(norm(u.user))).map(u => ({ ...u, inbound_answer_rate: u.inbound > 0 ? ((Number(u.inbound || 0) - Number(u.missed || 0)) / Number(u.inbound)) * 100 : null })); console.log("BENCHMARK_USER_COUNT", enriched.length, "TOTAL_SUMMARY_COUNT", userSummaries.length); return enriched; }, [userSummaries, benchmarkSet]);
  ...
  const handleUpload = async () => {
    ...
    try {
      // Call backend with parsed rows (file is already parsed on frontend)
  ...
  const exportPeriodExcel = async () => {
    ...
    const weekRows = sortedWeeks.map(week => {
      ...
      const rawInboundAnswered = totals.inbound_answered != null ? totals.inbound_answered : (totals.answered || 0);
      const inboundCount = totals.inbound || 0;
      const inboundAnswered = Math.min(rawInboundAnswered, inboundCount);
      return {
    ...
    userWeekRows.filter(u => !u._warning && (u.total_calls || 0) > 0).forEach(u => {
      pivotDataRows.push({
        ...
        answer_rate:            u.answer_rate !== null && u.answer_rate !== undefined ? Math.min(parseFloat((u.answer_rate * 100).toFixed(1)) / 100, 1) : "",
